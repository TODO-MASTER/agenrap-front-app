                ...
                <div className="flex flex-col w-full  gap-y-2  ">
                    ...
                    <div className="w-full flex-col">
                        {timeService.map(t => (
                            ...
                            <p key={t} className="font-tree font-semibold text-lg " >{timeUtils.toHourString(t)}</p>
                        ))}
                        <Slider value={timeService} onValueChange={setTimeService} className="bg-(--agenrap-purple-500) my-3" step={(3600 / 60) * 30} min={(3600 / 60) * 30} max={86400} />
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                <FormControl>
                                    <AgenrapInput variant="brownrap" placeholder="Ex. R$30,00"
                                        value={field.value}
                                        onChange={(e) => {
                    ...
                    }} className={`flex gap-x-2 justify-center items-center p-2 mb-2 ${timeService.length < 1 || stagingHasError || stagingEmpty ? "cursor-not-allowed opacity-70" : ""}`} >
                        <PlusCircle width={30} height={30} />
                        <p className="font-tree text-lg">Adicionar</p>
                    </AgenrapButton>

                    <AgenrapCollapsable variant={"brownrap"} collapseName="Seus serviços" spawnNotifier={
                        { haveNotifier: fields.length > 0, qtdNotifier: fields.length }
                    }>
                        {
                            fields.map((oc, indx) => (
