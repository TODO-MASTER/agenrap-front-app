...
import {
  initialBusinessServiceSchema,
  InitialBusinessServiceSchema,
} from "@/src/features/business/schemas"
...
export default function CreateOccupationForm() {
  ...
  const stagingHasError =
    !!errors.business?.staging?.name || !!errors.business?.staging?.price
  const stagingEmpty =
    !form.watch("business.staging.name") || !form.watch("business.staging.price")
  const canAdd = timeService >= 1 && !stagingHasError && !stagingEmpty

  const hasAtLeastOneAssigned = form.watch("business.occupations")?.some((o) => o.assignToMe)
  const canSubmit = form.formState.isValid && fields.length > 0 && hasAtLeastOneAssigned
  ...
  return (
    <Form {...form}>
      ...
      >
        ...
        <AgenrapButton
          ...
          onClick={async () => {
            const valid = await form.trigger([
              "business.staging.name",
              "business.staging.price",
            ])
            if (!valid) return
            const staging = form.getValues("business.staging")
            append({
              name: staging!.name,
              duration: timeService.toString(),
              price: staging!.price,
              assignToMe: true, // default marcado
            })
            form.resetField("business.staging")
            setTimeService((3600 / 60) * 30)
          }}
          className={`flex gap-x-2 justify-center items-center p-2 ${
        ...
        {fields.length > 0 && !hasAtLeastOneAssigned && (
          <p className="font-tree text-sm text-red-400 text-center">
            Marque pelo menos um serviço com “Eu atendo este serviço”
          </p>
        )}
          ...
          {serviceIsPending ? (
            <div className="flex relative">
              <Image
                src={macroLogo}
                alt=""
                className="w-10 h-10 opacity-15 animate-pulse"
              />
              <LoaderCircle
                className="animate-spin absolute w-10 h-10"
                color="#F5E6CC"
              />
            </div>
          ) : (
