...
export async function CreateANewService(values: CreateServiceReq, atSign: string) {
  const res = await serverAction<{ alreadyInitial: boolean; services: Service[] }>(
    `service/create?atSign=${normalizePublicHandle(atSign)}`,
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(values), // agora manda o objeto completo
    }
  )
