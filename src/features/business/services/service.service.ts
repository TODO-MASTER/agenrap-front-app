...
export async function CreateANewService(values:CreateServiceReq,businessName:string){
    const res = await serverFetch<CreateServiceRes>(`service/create?businessName=${businessName}`,{
        method:'POST',
...
export async function EditServiceService(values:Omit<Service,'id'>,rap:string,svsId:number) {
  const res = await serverFetch<EditServiceRes>(`service/edit?rap=${rap}&svsId=${svsId}`, {
    method: 'PUT',
    headers: { 'Content-Type': 'application/json' },
