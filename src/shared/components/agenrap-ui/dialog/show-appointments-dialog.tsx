...
export default function ShowAppointmentsDialog({
  open,
  onClose,
  appointments,
}: ShowAppointmentsDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent
        className="p-0 overflow-hidden gap-0 w-[calc(100%-2rem)] max-w-md rounded-2xl border-0 shadow-2xl shadow-black/30 bg-(--agenrap-gray-800)"
      ...
      >
        ...
        {/* Body */}
        <div className="px-4 py-4 bg-(--agenrap-gray-800)">
          {step === 'list' && (
          ...
          {step === 'confirm' && selected && (
            <div
              key="confirm"
              className="animate-in fade-in slide-in-from-right-4 duration-200 flex flex-col gap-4"
            >
              {/* Resumo */}
              <div className="flex items-stretch rounded-xl overflow-hidden bg-black/25 border border-white/5">
                <div
                ...
                <div className="flex flex-col justify-center gap-1 px-4 py-4 flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 text-xs">
                    <span className="text-(--agenrap-yellow-200) font-semibold">
                      {selected.appointmentHour.slice(0, 5)}
                    </span>
                    <span className="text-gray-600">·</span>
                    <span className="text-gray-400">
                      {selected.workingPeriodWeek}, {selected.appointmentDate}
                    </span>
                  </div>

                  {/* Profissional no confirm */}
                  {selected.professionalName && (
                    <div className="flex items-center gap-2 mt-2">
                      <ProfessionalAvatar
                        name={selected.professionalName}
                        color={selected.professionalAvatarColor ?? "slate"}
                        size="sm"
                      />
                      <div className="flex flex-col min-w-0">
                        ...
                        {selected.professionalTelephone && (
                          <span className="text-[10px] text-white/45 truncate">
                                  {selected.professionalTelephone?.trim() ? maskPhone(selected.professionalTelephone) : 'Sem telefone'}
                          </span>
                        )}
                      </div>
                    </div>
                ...
                <div className="flex items-center px-4 shrink-0">
                  <span className="text-sm font-bold text-(--agenrap-green-300)">
                    {(selected.serviceValue / 100).toLocaleString('pt-BR', {
                      style: 'currency',
                      currency: 'BRL',
                    })}
                  </span>
              ...
              {/* Aviso */}
              <div className="flex items-start gap-2 bg-(--agenrap-yellow-200)/10 border border-(--agenrap-yellow-200)/20 rounded-lg px-3 py-3">
                <AlertTriangle size={15} className="text-(--agenrap-yellow-200) mt-0.5 shrink-0" />
                <p className="text-xs text-(--agenrap-yellow-200)/80 leading-relaxed">
                  Esta ação não pode ser desfeita. O horário será liberado e o agendamento
        ...
        {/* Footer */}
        <div className="flex items-center justify-end gap-2 px-4 py-3 bg-(--agenrap-gray-800) border-t border-white/5">
          {step === 'confirm' && (
            <Button
              variant="ghost"
              onClick={handleBack}
              className="text-white/60 hover:bg-white/10 hover:text-white text-sm h-9"
            >
              Voltar
            </Button>
          )}
          ...
          <DialogClose asChild>
            <Button
              variant="outline"
              className="border-white/15 bg-transparent text-white/70 hover:bg-white/10 hover:text-white text-sm h-9"
            >
              Fechar
            </Button>
          </DialogClose>
            ...
            <AgenrapButton
              ...
              onClick={() =>
                handleCancelAppointmentAction(
                  ...
                  selected!.businessId,
                  usePathName === '/dashboard/customers' ? selected?.userId ?? null : null,
                  usePathName === '/dashboard/customers' ? selected?.customerId ?? null : null,
                  () => {
                    setStep('list')
                    router.refresh()
                    onClose()
